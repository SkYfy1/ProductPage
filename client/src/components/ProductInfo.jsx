...
const ProductInfo = ({ prod, show }) => {
    ...
    const reviews = prod.reviews.lenght === 0 ? 0 : prod.reviews.reduce((acc, next) => acc + next.stars, 0)
    ...
    return (
        <div>
            ...
            <div className='flex gap-2 items-center mb-4 mt-2 text-center'>
                ...
                <Rate data={Math.ceil(reviews / prod.reviews.length)} />
                <a href="" onClick={(e) => { e.preventDefault(); show() }} className='text-blue-800 text-xs font-semibold'>See all reviews</a>
            </div>
